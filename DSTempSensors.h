...
const CaliT CalibrationTable[] = {
  ...
  { .addr = {0x28, 0x01, 0x8E, 0x79, 0x97, 0x11, 0x03, 0xE3}, .offset = 1.0},
...
class DSTempSensor {
  ...
  public:
    ...
    float getTempC();
    /**
      Don't understand copy,=,move enough to implement
      for emplace (vector) tried to do this, but seems defaults are ok
         DSTempSensor (const DSTempSensor &obj) {
           Serial.println("COPY");
         }

      DSTempSensor(DSTempSensor&& other)         {
        Serial.println("MOVE");
      }
      DSTempSensor& operator=(const DSTempSensor& other) = default;
    */
  ...
  private:
    ...
    DeviceAddress _addr; // = {0, 0, 0, 0, 0, 0, 0, 0}; // call sensor by addr - faster
    char _addrCStr[17]; //AddrLen * 2 + 1
    const char* getAddrCStr(DeviceAddress addr);
    DallasTemperature *_sensorBus;
};

class DSTempSensors: public Sensor {

  public:
    DSTempSensors(QueueHandle_t _readingsQueue, int pin);
    void begin();
    void read();
  ...
  private:
    OneWire *_oneWire;// Setup a oneWire instance to communicate with any OneWire devices (not just Maxim/Dallas temperature ICs)
    DallasTemperature *_sensorBus;        // instance of OneWire bus
    vector<DSTempSensor> _sensors;  // store array of addresses, as callilng by address is faster
