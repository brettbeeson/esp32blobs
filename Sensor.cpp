...
Sensor::Sensor(QueueHandle_t readingsQueue)
  :  _readingsQueue(readingsQueue), _taskPeriodMs(10000) {
}
...
void Sensor::taskify(int taskPeriodMs, int priority) {
  ...
  Log.verbose("Sensor executing on core %d", core);
...
void Sensor::copyReadingsToQueue() {
  ...
  for (int i = 0; i < nReadings(); i++) {
    ...
    if (xStatus == pdPASS) {
      ...
      delete(r); r = NULL;
      Log.error("Could not send to the queue. Full = %d", xStatus == errQUEUE_FULL);
...
void Sensor::clearReadings() {
  for (int i = 0; i < nReadings(); i++) {
    ...
    // readings[i]->value=NAN; timestamp!
  }
}
